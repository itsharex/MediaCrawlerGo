...
// loginByCookies login by scan qrcode image
func (xl *XhsLogin) loginByQrcode() {
	...
	if len(base64QrcodeImg) == 0 {
		util.Log().Info("login failed , have not found qrcode please check ....")
		...
		if err != nil {
			util.Log().Error("failed to click login button")
			return
		}
		base64QrcodeImg := findLoginQrcode(xl.contextPage, qrcodeImageSelector)
		if len(base64QrcodeImg) == 0 {
	...
	if err != nil {
		util.Log().Error("show qrcode error")
		return
	}
